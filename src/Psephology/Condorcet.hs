...
module Psephology.Condorcet (condorcetWinner, condorcetMatrix, numPreferOver, smithSet) where
...
condorcetWinner candidates voters =
    let winningScore = fromIntegral $ length candidates - 1
     in findIndex ((==) winningScore . copelandScore candidates voters) [0 .. length candidates - 1]
...
copelandScore candidates voters c =
    sum [copelandij $ pairwiseMaj voters (candidates !! c) (candidates !! b) | b <- [0 .. length candidates - 1], c /= b]
...
copelandSet candidates voters =
    let copelandScores = map (copelandScore candidates voters) [0 .. length candidates - 1]
     ...
     in filter (\i -> maxCopelandScore == copelandScores !! i) [0 .. length candidates - 1]
...
smithSet candidates voters =
    foldl (smithSetStep candidates voters) (copelandSet candidates voters) [0 .. length candidates - 1]

smithSetStep :: (Voter a) => [Candidate] -> [a] -> [Int] -> Int -> [Int]
smithSetStep candidates voters set i
    | i `elem` set = set
    | any (\c' -> pairwiseMaj voters c c' >= 0) candidates = i : set
    | otherwise = set
  where
    c = candidates !! i
